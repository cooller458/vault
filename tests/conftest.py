...
@pytest.fixture
def vault(WBTVault, WBTStrategy, pool, router, tokens, gov, users, keeper):
    # protocolFee = 10000 (1%)
    ...
    strategy = gov.deploy(WBTStrategy, vault, 2400, 1200, 200000, 600, keeper)
...
@pytest.fixture
def strategy(WBTStrategy, vault):
    return WBTStrategy.at(vault.strategy())
...
def createPoolVaultStrategy(
    pm, WBTVault, WBTStrategy, MockToken, router, gov, keeper, users
):
    UniswapV3Core = pm(UNISWAP_V3_CORE)
